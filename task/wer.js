...
class PersonValidate {
    ...
    validatename() {
        if (nameRegex.test(this.person.name.trim())) {
            console.log("Method 1 called", this.person.name);
    ...
    validateage() {
        if (ageRegex.test(this.person.age)) {
            console.log("Method 2 called with message");
...
// Iterate through property names and call methods (excluding constructor)
propertyNames.forEach(propertyName => {
    if (propertyName !== 'constructor' && typeof PersonValidate.prototype[propertyName] === 'function') {
        if (validator.errors.length > 0) {
            return;
        }
        PersonValidate.prototype[propertyName].call(validator); // Call the method on `validator`
    }
});
